function isOneEditDistance(s: string, t: string): boolean {
  ...
  const removeSymbol = (str: string, idx: number): string =>
    str
      .split("")
      .filter((s, id) => id !== idx)
      .join("");

  const replaceSymbol = (str: string, idx: number, sym: string): string =>
    str
      .split("")
      .map((s, id) => (id === idx ? sym : s))
      .join("");
...
console.log(
  "!!SHOULD BE TRUE isOneEditDistance('ab', 'acb')",
  isOneEditDistance("ab", "acb")
); // true (because of diff === 1)
console.log(
  "!!SHOULD BE FALSE isOneEditDistance('abd', 'acb')",
  isOneEditDistance("abd", "acb")
); // false (because of diff === 2)
console.log(
  "SHOULD BE TRUE !!isOneEditDistance('aca', 'caca')",
  isOneEditDistance("aca", "caca")
); // true (because of diff === 1)
console.log(
  "!!SHOULD BE FALSE isOneEditDistance('ab', 'ba')",
  isOneEditDistance("ab", "ba")
); // false (because of diff === 2)
console.log(
  "!!SHOULD BE TRUE isOneEditDistance('a', 'ac')",
  isOneEditDistance("a", "ac")
); // true (because of diff === 1)
